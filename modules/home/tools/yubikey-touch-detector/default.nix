...
# TODO 20241116 Move the package portion to packages/
{
  options,
  config,
  lib,
...
with lib.${namespace};
let
  cfg = config.${namespace}.tools.yubikeyTouchDetector;
in
{
  options.${namespace}.tools.yubikeyTouchDetector = with types; {
    enable = mkBoolOpt false "Whether or not to enable yubikeyTouchDetector, a tool to detect when your YubiKey is waiting for a touch";
